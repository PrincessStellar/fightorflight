...
public class PokemonArrowRenderer extends EntityRenderer<PokemonArrow> {
    ...
    public void render(PokemonArrow entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        float g = 0.15625F;
        float h = 0.3125F;
        float scaleMultiplier = 0.05625F;
        ...
        this.vertex(pose, vertexConsumer, -7, -2, -2, f, g, -1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, -2, 2, g, g, -1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, 2, 2, g, h, -1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, 2, -2, f, h, -1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, 2, -2, f, g, 1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, 2, 2, g, g, 1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, -2, 2, g, h, 1, 0, 0, packedLight, color);
        this.vertex(pose, vertexConsumer, -7, -2, -2, f, h, 1, 0, 0, packedLight, color);
        ...
        for (int r = 0; r < 4; ++r) {
            ...
            this.vertex(pose, vertexConsumer, 8, 2, 0, 0.5F, g, 0, 1, 0, packedLight, color);
            this.vertex(pose, vertexConsumer, -8, 2, 0, 0.0F, g, 0, 1, 0, packedLight, color);
        }
